...
#配置文件目录 /etc/influxdb/influxdb.conf
#配置文件参考文档网址 https://docs.influxdata.com/influxdb/v0.9/guides/clustering/
...
echo "set  [/etc/influxdb/influxdb.conf] model [meta] param hostname=ip manual"
echo "service influxdb start; service influxdb stop"
echo "echo -n "id" > /data/influxdb/meta/id "

#启动需要做的工作
1) /etc/influxdb/influxdb.conf 修改 ip
	
	 [meta] param hostname=ip
2）
  service influxdb start; service influxdb stop
  
3）配置实例 id
  echo "echo -n "id" > /data/influxdb/meta/id "

3）配置整个集群的实例 /data/influxdb/meta/peers.json

	["ip1:8088","ip2:8088","ip3:8088","ipN:8088"]
...
配置集群后命令行执行 influx 就可以像连接mysql 一样连接到server
show servers 查看集群所有实例节点
